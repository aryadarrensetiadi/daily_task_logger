...
This script sends a "/result" command to the Telegram group/topic
to request the result or summary output from the bot.

Steps:
1. Send the /result command to the group/topic
...
# === Send /result command to Telegram ===
with TelegramClient(session_path, api_id, api_hash) as client:
    client.send_message(group_id, "/result", reply_to=topic_id)
    print("📨 Sent command: /result")
    ...
    if not has_response:
        ...
        time.sleep(1)  # Let session close cleanly
