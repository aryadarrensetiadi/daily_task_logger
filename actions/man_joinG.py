...
This script allows a user to request joining a group in the bot system
by sending a "/joingrup <group_id>" command to a specified Telegram thread.

Steps:
1. Prompt user to input a group ID (numbers only)
2. Send the /joingrup command to the group/topic
...
def run_join_group(user_input: str):
...
if __name__ == "__main__":
    user_input = input("Enter group ID to join (only numbers): ").strip()
    run_join_group(user_input)
