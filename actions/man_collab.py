...
This script allows a user to request a collaboration action by sending
a "/collab <collab_id>" command to a specified Telegram thread.

Steps:
1. Prompt user to input a collab ID (numbers only)
2. Send the /collab command to the group/topic
...
if __name__ == "__main__":
    user_input = input("Enter collab ID (only numbers): ").strip()
    run_collab(user_input)
